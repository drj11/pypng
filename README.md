...
`gen` now uses centre pixel sampling, which means that a 256 pixel
wide 8-bit gradient takes on all possible 256 values.
It also improves output for very small images.

`gen` now uses `Fraction` for internal maths meaning that the stripe
patterns are accurate and do not have loose pixels.

`gen` now only outputs greyscale PNG files
(but see next item for a feature to generate colour PNGs).

`gen` can output multiple PNGs onto the same stream.
...
piping `gen` into `priweavepng`:
`gen RTL RTR RBR | priweavepng - - -`
